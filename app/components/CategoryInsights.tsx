...
export default function CategoryInsights() {
  ...
  const [data, setData] = useState<{
    best_selling: CategoryStat[];
    lowest_selling: CategoryStat[];
    all_categories: CategoryStat[];
  } | null>(null);
  ...
  useEffect(() => {
    async function fetchCategoryData() {
      ...
      try {
        ...
        const res = await fetch(buildApiUrl(`/api/v1/analytics/categories?${queryParams.toString()}`));
        if (res.ok) {
          const result = await res.json();
          setData(result);

          // Auto-select first category as default profile
          if (result.best_selling && result.best_selling.length > 0) {
            setSelectedCategory(result.best_selling[0]);
          }
        } else {
          setError(true);
        }
      } catch (err) {
        console.error("Failed to fetch categories analytical data", err);
        setError(true);
      } finally {
        setLoading(false);
      }
    ...
    async function fetchTopProducts() {
      ...
      try {
        const res = await fetch(
          buildApiUrl(
            `/api/v1/analytics/categories/top-products?category_name=${encodeURIComponent(selectedCategory.category)}`
          )
        );
        if (res.ok) {
          const result = await res.json();
        ...
        console.error("Failed to load top products for category", err);
      } finally {
    ...
    return (
      <div className="glass-card p-8 flex flex-col items-center justify-center h-48 text-center">
        ...
        <span className="text-sm font-semibold text-foreground">
          Failed to load category analytics data
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-5">
        ...
        <div className="relative w-full">
          ...
          <input
            ...
            placeholder="Type category name or code to filter and select category analytics..."
          ...
          {/* Category dropdown drawer when search query is typing - Positioned absolutely to float */}
            ...
            <div className="absolute left-0 right-0 mt-1 border border-border rounded-lg max-h-60 overflow-y-auto z-50 shadow-2xl bg-card">
              {filteredCategories.length > 0 ? (
                filteredCategories.map((item) => (
                  ...
                  >
                    {item.category} (Sales: {formatCurrency(item.sales)})
                  </button>
                ))
              ) : (
      ...
      {/* Category detailed Profile Panel & Metrics (Side-by-Side Cards) */}
      {selectedCategory && (
            ...
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className="text-xs font-bold text-foreground font-mono">
                  {formatCurrency(selectedCategory.sales)}
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
              ...
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className={cn("text-xs font-bold font-mono",
                  selectedCategory.margin_pct > 0 ? "text-emerald-500" : selectedCategory.margin_pct < 0 ? "text-rose-500" : "text-muted-foreground"
                )}>
                  {(selectedCategory.margin_pct || 0).toFixed(2)}%
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                <span className="text-xs text-muted-foreground">Active SKUs</span>
                <span className="text-xs font-bold text-indigo-500 font-mono">
                  {selectedCategory.sku_count || 0} Products
                </span>
              </div>
              <div className="flex justify-between pb-1">
                ...
                <span className="text-xs font-bold text-sky-500 font-mono">
                  {selectedCategory.stock_qty.toLocaleString()} Units
                </span>
              </div>
            </div>
          </div>

          {/* Category Logistics KPIs */}
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            <div>
            ...
            <div className="space-y-3.5 mt-6">
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className="text-xs font-bold text-emerald-500 font-mono">
                  {(selectedCategory.receive_qty || 0).toLocaleString()} Units
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className="text-xs font-bold text-rose-500 font-mono">
                  {(selectedCategory.return_qty || 0).toLocaleString()} Units
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span
                  className={cn(
                    ...
                    selectedCategory.return_rate_pct > 0 ? "text-emerald-500" : selectedCategory.return_rate_pct < 0 ? "text-rose-500" : "text-muted-foreground"
                  )}
                >
                  {(selectedCategory.return_rate_pct || 0).toFixed(2)}%
                </span>
              </div>
              <div className="flex justify-between border-b border-border pb-2">
                ...
                <span className="text-xs font-bold text-foreground font-mono">
                  {formatCurrency(
                    (selectedCategory.sales || 0) / (selectedCategory.sku_count || 1)
                  )}
                </span>
              </div>
              <div className="flex justify-between pb-1">
                ...
                <span
                  className={cn(
                    ...
                    selectedCategory.margin_pct > 12
                      ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                      : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                  )}
                >
                  {selectedCategory.margin_pct > 12 ? "High Margin" : "Low Margin"}
                </span>
              </div>
            </div>
          </div>

          {/* Top 5 Products of Selected Category */}
          <div className="glass-card p-6 flex flex-col justify-between min-h-[350px]">
            <div>
            ...
            {loadingProducts ? (
              ...
              <div className="flex-1 flex items-center justify-center text-xs text-muted-foreground min-h-[150px]">
                No sales records mapped for this category.
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground">Top 10 Best Selling Categories</h3>
              ...
              <table className="w-full text-left text-xs font-mono border-collapse">
                <thead>
                  <tr className="border-b border-border bg-muted/50 sticky top-0 z-10">
                    <th className="p-2.5 text-muted-foreground font-semibold">Category</th>
                  ...
                  {data?.best_selling.map((item, idx) => (
                    <tr
                      ...
                      className={cn(
                        ...
                        selectedCategory?.category === item.category && "bg-primary/5 font-bold"
                      )}
                    >
        ...
        <div className="glass-card p-6 flex flex-col justify-between h-[440px]">
          <div className="flex items-center gap-2.5 mb-4">
            ...
            <div>
              <h3 className="text-sm font-bold text-foreground">Top 10 Low Selling Categories</h3>
